...
class PathWayGnnManager(object):
    ...
    def evaluate(self, model, data, mode="test_data"):
        ...
        if mode == "test_data":
            ...
            label_list_acc = label_list.flatten()
            t_list_acc = t_list.flatten()
        ...
        auc = accuracy_score(t_list_acc, label_list_acc)
    ...
    def model_test(self, data, model, test_epoch):
        ...
        p_properties_acc = p_properties.flatten()
        t_properties_acc = t_properties.flatten()
    ...
    def run_model(self,
        ...
        for epoch in range(1, epochs + 1):
            ...
            auc_test, precision_test, recall_test, f1score_test = self.evaluate(model,
                                                                                data,
                                                                                mode="test_data")
            precision_dev_list.append(precision_val)
        ...
        if test_mode:
            ...
            avg_auc, avg_pre, avg_rec, avg_f1, class_information_list = self.model_test(data, model, test_epoch)

            return avg_auc, avg_pre, avg_rec, avg_f1, class_information_list
        precision_mean = np.mean(precision_dev_list)
        return auc_val, precision_mean, recall_val, f1score_val
