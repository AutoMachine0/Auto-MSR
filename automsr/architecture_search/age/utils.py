...
def experiment_graphpas_data_save(path,
    ...
    with open(path + "/" + file_name, "w") as f:
        ...
        for gnn_architecture,  val_performance, in zip(performance_list, performance_list):
...
def parent_random_sample(pop,
                         pop_performance,
                         num):

    choice_index_list = [i for i in range(len(pop))]

    parent_index = np.random.choice(choice_index_list,
                                    num,
                                    replace=False)
    parent = []
    parent_performance = []
    ...
    for index in parent_index:
        ...
        parent_performance.append(pop_performance[index])

    parent, _ = top_population_select(parent,
                                      parent_performance,
                                      num)
...
def save_best_gnn_architecture(gnn_architecture_list,
                               path,
                               file_name):
    with open(path + "/" + file_name, "w") as f:
        f.write(str(gnn_architecture_list))
